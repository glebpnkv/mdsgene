...
class ExcelMappingApp:
    ...
    # --- ИЗМЕНЕНО: __init__ с поддержкой documents, document_id и data_query ---
    def __init__(
        self,
        pdf_filepath: Path,
        model_name: str,
        ...
        Args:
            ...
            document_id: ID of the document in the documents dictionary
            documents: Dictionary of documents
            vector_store: Optional vector_store instance for retrieving full document text
        """
        ...
        self.vector_store = vector_store  # Store the DataQuery instance
        self.full_text: str = ""      # Will store the full document text
        ...
        if not list_of_patient_question_sets:
            ...
            for patient_question_set in list_of_patient_question_sets:
                ...
                for q_obj in patient_question_set:
                    ...
                    # First try to use DataQuery if available
                    if self.vector_store:
                        try:
                            raw_answer = str(self.vector_store.similarity_search(query_text, k=1)[0].page_content)
                        except Exception as data_query_err:
                            self.log(f"ERROR using DataQuery: {data_query_err}. Falling back to alternatives.",
                                  level="ERROR", error=True)
                            # Fall back to cached answer or gemini_processor
                            if cached_answer is not None:
                                raw_answer = cached_answer
                            elif hasattr(self.gemini_processor, 'answer_question'):
                                raw_answer = self.gemini_processor.answer_question(query_text)
                            else:
                                self.log("ERROR: All answer methods failed.",
                                      level="ERROR", error=True)
                                raw_answer = {"answer": "-99_API_METHOD_MISSING", "evidence": ""}
                    # If DataQuery not available, use cached answer or gemini_processor
                        ...
                        try:
                            if hasattr(self.gemini_processor, 'answer_question'):
                                ...
                                self.log("ERROR: Neither DataQuery nor GeminiProcessor available for answering questions.",
                                      level="ERROR", error=True)
                                raw_answer = {"answer": "-99_API_METHOD_MISSING", "evidence": ""}
                    ...
                    # Cache the answer regardless of which method was used to get it
