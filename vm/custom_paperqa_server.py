...
def init_paperqa():
    ...
    try:
        ...
        # --- MODIFICATION START ---
        # Define custom router kwargs to override lmi defaults
        custom_router_kwargs = {
            "num_retries": 3,     # Keep the retry count
            "timeout": OLLAMA_TIMEOUT  # Use the timeout from your script's config
        }
        logger.info(f"Setting custom router_kwargs with timeout: {OLLAMA_TIMEOUT}s")
        # --- MODIFICATION END ---
        ...
        settings_instance = Settings( # Assign to global settings_instance
            ...
            llm_config={
                "model_list": [
                    {
                        "model_name": f"ollama/{PAPERQA_MODEL_NAME}",
                        "litellm_params": {
                            "model": f"ollama/{PAPERQA_MODEL_NAME}",
                            "api_base": ollama_service_base,
                            # Timeout here is likely overridden by router_kwargs, but included for clarity
                            "timeout": OLLAMA_TIMEOUT
                        }
                    }
                ],
                "router_kwargs": custom_router_kwargs # <-- Apply custom router_kwargs
            },
            summary_llm=f"ollama/{FORMATTER_MODEL_NAME}", # Use formatter model for summary too, or define separately
            summary_llm_config={
                "model_list": [
                    {
                        "model_name": f"ollama/{FORMATTER_MODEL_NAME}",
                        "litellm_params": {
                            "model": f"ollama/{FORMATTER_MODEL_NAME}",
                            "api_base": ollama_service_base,
                            "timeout": OLLAMA_TIMEOUT
                        }
                    }
