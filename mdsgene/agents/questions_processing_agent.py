...
class QuestionsProcessingAgent(BaseAgent[State]):
    ...
    def save_to_cache(self, cache_identifier: str, raw_answer: str, question: str = None, context: str = None, processor: str = "gemini"):
        """
        ...
        Args:
            ...
            processor: The processor used to generate the answer (default: "gemini")
        """
        # Get the PMID-specific cache directory
        pmid_dir = CACHE_DIR / self.pmid if self.pmid else CACHE_DIR
        ...
        try:
            cache_data = {"raw_answer": raw_answer, "processor": processor}
            if question:
    ...
    def process_patient_questions(self, state: State) -> State:
        ...
        if cached_data is not None:
            ...
            raw_answer = None
            try:
                processor_name = os.getenv("PROCESSOR_NAME", "gemini")
                ...
                if result:
                    raw_answer = result[0]
                    self.save_to_cache(cache_identifier, raw_answer, prompt, result[1], "gemini")
    ...
    def batch_format_patient_results(
        ...
        if missing_ids:
            ...
            # Get batch size from environment variable or use default value of 3
            try:
                batch_size = int(os.getenv('PATIENT_BATCH_SIZE', 3))
                if batch_size <= 0:
            ...
            for batch in batches:
                try:
                    ...
                    if not formatted_json_text:
                        print("ERROR: Gemini formatting response is empty:\n---")
                    ...
                    if json_match:
                        ...
                        # If not a code block and not starting with {, try to find JSON
                        match = re.search(r"\{.*\}", text, re.DOTALL)
                        if match:
                            text = match.group(0)
                        else:
                            print("ERROR: Gemini formatting response is not JSON:\n---")
                            print(formatted_json_text)
                            continue
