...
async def process_to_excel(data: List[Dict[str, Any]] = Body(...)):
    ...
    for patient in data:
            ...
            if field in EXPECTED_HEADERS:
                ...
                if field in field_processors:
                    ...
                    if hasattr(CustomProcessors, processor_name):
                        ...
                        if processor_name == "mdsgene_decision":
                            # Call after all fields are filled
                            continue  # Skip for now, will process after the loop
                        else:
        ...
        # ... (after the for field, value in patient.items() loop)
        if "mdsgene_decision" in field_processors.values():
            for field, processor_name in field_processors.items():
                if processor_name == "mdsgene_decision" and field in EXPECTED_HEADERS:
                    processor_method = getattr(CustomProcessors, processor_name)
                    row[field] = processor_method(patient.get(field, '-99'), row)
